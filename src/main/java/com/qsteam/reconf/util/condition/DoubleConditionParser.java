 ...
 *
 * <p>For integer-only conditions where exact {@code long} precision matters
 * (e.g. IDs, counters, timestamps beyond 2^53), use {@link LongConditionParser}
 * instead: routing large integers through a {@code double} bound can lose
 * precision, which this parser does not attempt to avoid.
 *
 ...
 * <h2>Example</h2>
 * <pre>{@code
 * DoublePredicate condition = DoubleConditionParser.parse("(..85] & !{5} | [100..)");
 ...
 * }</pre>
 */
public class DoubleConditionParser extends AbstractConditionParser<DoublePredicate> {
    ...
    public static DoublePredicate parse(String expr) {
        DoubleConditionParser parser = new DoubleConditionParser();
